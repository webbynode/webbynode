...
describe Webbynode::Commands::Remote do
  ...
  def load_all_mocks(rem=remote)
    rem.stub(:remote_executor).and_return(re)
    rem.stub(:git).and_return(git)
    rem.stub(:io).and_return(io)
    rem.stub(:pushand).and_return(pushand)
    rem.stub(:server).and_return(server)
