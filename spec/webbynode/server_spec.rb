...
describe Webbynode::Server do
  ...
  describe "#add_ssh_key" do
    before(:each) do
      ...
      @re = double("RemoteExecutor")
      @re.as_null_object

      @pushand = double("PushAnd")
      @pushand.as_null_object

      @server = Webbynode::Server.new("1.2.3.4", "git", 22)
      @server.stub(:io).and_return(@io)
      @server.stub(:remote_executor).and_return(@re)
      @server.stub(:pushand).and_return(@pushand)
    end
